...
* [ArchiTechAnon's ProTV](https://architechvr.gumroad.com/l/protv) 3 or newer, via ProTV's native LTCGI integration

After importing USharpVideo, LTCGI will enable its USharpVideo adapter automatically in `Assets/_pi_/_LTCGI-Adapters`. Your LTCGI Controller should then detect video players in the scene automatically (a display box should appear). Press the offered button on the controller to perform the automatic setup.

For ProTV 3 or newer, use ProTV's native LTCGI integration. If an old LTCGI ProTV adapter is detected, LTCGI will warn you and can remove the outdated adapter files.
